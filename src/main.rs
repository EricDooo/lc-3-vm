...
impl LC3 {
    ...
    fn update_flags(&mut self, r: usize) {
        ...
        } else if (value >> 15) != 0 { // Check if the sign bit is set
    ...
    fn read_image_file<P: AsRef<Path>>(&mut self, path: P) -> io::Result<()> {
        ...
        match op {
            op if op == OpCode::ADD as u8 => {
                ...
                if (instr & 0x20) == 0 {
                    // Register mode
                    let sr2 = instr & 0x7;
                    self.registers[dr as usize] = self.registers[sr1 as usize].wrapping_add(self.registers[sr2 as usize]);
                } else {
                    // Immediate mode
                ...
                match instr & 0xFF {
                    ...
                    trap if trap == TrapCode::PUTSP as u16 => {
                        ...
                        loop {
                            ...
                            if char2 != 0 {  // Check if second character is null terminator
